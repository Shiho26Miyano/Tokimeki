...
// Poetry Generator (HuggingFace)
async function generatePoem() {
    const termInput = document.getElementById('poetry-term');
    const resultDiv = document.getElementById('poetry-result');
    const attributionDiv = document.getElementById('poetry-attribution');
    const term = termInput.value.trim();
    if (!term) {
        resultDiv.innerHTML = '<span style="color:red;">Please enter a word or phrase.</span>';
        attributionDiv.innerHTML = '';
        return;
    }
    resultDiv.innerHTML = '<span style="color:#888;">Generating poem...</span>';
    attributionDiv.innerHTML = '';
    try {
        const res = await fetch('/poetry/generate', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (data.error) {
            ...
            // Display poem lines
            resultDiv.innerHTML = data.poem && data.poem.length > 0
                ? data.poem.map(line => `<div>${line}</div>`).join('')
                : '<span style="color:#888;">No poem generated.</span>';
            attributionDiv.innerHTML = data.source ? `Poem generated by <b>HuggingFace GPT-2</b> via HuggingFace Inference API.` : '';
        }
    } catch (e) {
        resultDiv.innerHTML = `<span style='color:red;'>Failed to generate poem. Please try again later.</span>`;
        attributionDiv.innerHTML = '';
    }
}
